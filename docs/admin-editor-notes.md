...
- While dragging within the same section, the original block is visually hidden and a same-sized floating preview moves to the intended logical grid cell before mouseup. The preview is absolutely positioned, sized from the shared grid column/row formulas, and does not participate in CSS grid layout, so nearby blocks do not shrink, disappear, or get reflowed just because the preview moved.
